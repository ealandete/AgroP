...
function validateInvoiceForm(fForm, items) {
  const errors = {}
  if (!fForm.cliente_id) errors.cliente_id = 'El cliente es obligatorio'
  if (items.length === 0) errors.items = 'Debe agregar al menos un item'
  return errors
}

function validateMovForm(movForm) {
  const errors = {}
  if (movForm.monto !== '' && movForm.monto !== null && Number(movForm.monto) <= 0) errors.monto = 'El monto debe ser mayor a 0'
  return errors
}
...
export default function Contabilidad() {
  ...
  const isInvoiceValid = Object.keys(validateInvoiceForm(fForm, items)).length === 0

  const handleEmitir = async () => {
    const errors = validateInvoiceForm(fForm, items)
    setFacturaErrors(errors)
    if (Object.keys(errors).length > 0) return
    try {
      await api.post('/facturacion/', { ...fForm, items })
      notifications.show({ title: 'Factura emitida', color: 'green' })
      close(); setItems([]); setFacturaErrors({}); loadData()
  ...
  const isMovValid = Object.keys(validateMovForm(movForm)).length === 0

  const handleCrearMovimiento = async () => {
    const errors = validateMovForm(movForm)
    setMovErrors(errors)
    if (Object.keys(errors).length > 0) return
  ...
  return (
    <Stack>
      ...
      <Group><Button leftSection={<IconPlus size={16} />} onClick={() => { setItems([{ producto_id: '', descripcion: '', cantidad: '', precio_unitario: '', unidad_medida: 'unidad', iva_porcentaje: 19 }]); setFacturaErrors({}); open() }}>Nueva Factura</Button></Group>
      ...
      <Tabs defaultValue="facturas">
        ...
        <Tabs.Panel value="diario" pt="md">
          <Group mb="sm">
            <Button leftSection={<IconPlus size={16} />} size="sm" onClick={() => { setMovForm({ tipo: '', fecha: new Date().toISOString().split('T')[0], cuenta_contable_id: '', descripcion: '', monto: '', medio_pago: '', referencia: '' }); setMovErrors({}); openMovimiento() }}>Nuevo Movimiento</Button>
          </Group>
          <div style={{ overflowX: 'auto' }}>
            <Table striped highlightOnHover>
        ...
        <Stack>
          <SimpleGrid cols={{ base: 1, sm: 3 }}>
            <Group align="end" gap="xs" wrap="nowrap">
              <Select label="Cliente *" data={clientes.map(c => ({ value: c.id.toString(), label: c.nombre }))} value={fForm.cliente_id} onChange={v => setFForm({ ...fForm, cliente_id: v })} required searchable style={{ flex: 1 }} error={facturaErrors.cliente_id} />
          ...
          {facturaErrors.items && <Text c="red" size="sm">{facturaErrors.items}</Text>}
          {items.map((it, i) => (
          ...
          <Group justify="flex-end"><Button variant="default" onClick={close}>Cancelar</Button><Button onClick={handleEmitir} disabled={!isInvoiceValid}>Emitir Factura</Button></Group>
        </Stack>
      </Modal>
      ...
      <Modal opened={openedMovimiento} onClose={closeMovimiento} title="Nuevo Movimiento" size="md">
        <Stack>
          ...
          <NumberInput label="Monto *" value={movForm.monto} onChange={v => setMovForm({...movForm, monto: v})} min={0} error={movErrors.monto} />
          ...
          <Group justify="flex-end"><Button variant="default" onClick={closeMovimiento}>Cancelar</Button><Button onClick={handleCrearMovimiento} loading={loadingMovimiento} disabled={!isMovValid || !movForm.tipo || !movForm.cuenta_contable_id}>Guardar Movimiento</Button></Group>
        </Stack>
      </Modal>
